        PROGRAM Dynamics_Cal
        USE parm
        USE dimensionless_parm
        USE var_type
        IMPLICIT NONE
        EXTERNAL f

        REAL*8,PARAMETER:: qmin = -3.d0, qmax = 3.d0
        REAL*8,PARAMETER:: pmin = -4.d0, pmax = 4.d0

        OPEN(UNIT=11,FILE="ic/initial.in")
        !OPEN(UNIT=13,FILE="energy.dat")
        !OPEN(UNIT=50,FILE="j_th_hf.dat")
        OPEN(UNIT=60,FILE="stb_plot.dat")
        OPEN(UNIT=70,FILE="time_avgJ.dat")
        !OPEN(UNIT=22,FILE="Diss_Stat/stat_diss_traj.dat")
        OPEN(UNIT=100,FILE="Output.dat")
...
c       READ(11,*)qx,px
        qx= qmin + (qmax-qmin)*RAND(0)
        px= pmin + (pmax-pmin)*RAND(0) 

        ystart(1)= qx            
        ystart(2)= px  
c          PRINT*,ystart(1),ystart(2)          

        !...............call setup................

          CALL SETUP(NEQ,TSTART,YSTART,TEND,TOL,THRES,METHOD,TASK, 
     &                ERRASS,HSTART,WORK,LENWRK,MESAGE)
    
          stepsize=(tend-tstart)/DFLOAT(iter)

          DO i=1,iter
            twant =  dble(i)*stepsize

10         CALL UT(F,TWANT,TGOT,YGOT,YPGOT,YMAX,WORK,UFLAG)
           IF(uflag.ne.1)THEN
                go to 10
           END IF
           t=tgot  
           ...
           WRITE(100,fmt1) t,x_hf,p_hf
c            PE = D_eff*(1.d0-DEXP(-delta*ygot(1)))**2 
c      &                  +D_eff*(1.d0+epsiln*DCOS(t))*ygot(1)
c            KE = 0.5d0*(ygot(2))**2
c            E_HF = PE + KE 
           
c         IF(E_HF/D_eff .LT. 1.d0)THEN
c            act_hf(j,i)=(DSQRT(2.d0*D_eff)/delta)*
c      &          (1.d0-(DSQRT(1.d0-(E_HF/D_eff))))

c            IF(px .GE. 0.d0)THEN
c                 th_HF=-1.d0*DACOS(((1.d0-E_HF/D_eff)/DSQRT(E_HF
c      &  /D_eff))*DEXP(delta*(ygot(1)))-1.d0/DSQRT(E_HF/D_eff))
c            ELSE
c                 th_HF=1.d0*DACOS(((1.d0-E_HF/D_eff)/DSQRT(E_HF
c      &  /D_eff))*DEXP(delta*(ygot(1)))-1.d0/DSQRT(E_HF/D_eff))
c            END IF
...
c         !WRITE(50,*)t,act_hf,th_HF
         IF(MOD(i,200).EQ.0)THEN
           WRITE(60,*)ygot(1),ygot(2) 

         END IF
c !!..............................................................!!
c            IF(ygot(1) .GT. 20.d0)THEN
c                 diss_traj=diss_traj+1
c                 PRINT*,t
c                 EXIT
c            END IF
!!..............................................................!!
         END DO
        END DO
c         WRITE(22,*)diss_traj,DFLOAT(diss_traj)/DFLOAT(ntraj)
c      &                  , ntraj-diss_traj
        
c         DO m=1,iter
c         avg_jhf = 0.d0
c           DO n=1,ntraj
c             avg_jhf = avg_jhf + act_hf(n,m)
c           END DO
c           avg_jhf = avg_jhf/ntraj
c           WRITE(70,"(8F16.6)")m*stepsize,avg_jhf
c         END DO
c         DEALLOCATE(act_hf)
        END PROGRAM
        ...
        SUBROUTINE sos(i,x,y,px,py,x1,x2,p1,p2)
        IMPLICIT NONE
        REAL*8,INTENT(IN)::x,y,px,py
        INTEGER,INTENT(IN)::i
        REAL*8,INTENT(OUT)::x1,x2,p1,p2
        REAL*8::x0,y0,px0,py0
        ...
        USE dimensionless_parm
        IMPLICIT NONE
        REAL*8::y(neq),yp(neq),t
        REAL*8::dx_VHF,ddx_VHF
        REAL*8::hbar_eff,D_eff,L_eff,delta

        hbar_eff = hbar_eff_function()
        D_eff = D_eff_function()
        L_eff = L_eff_function()
        delta = a_hf*L_eff

c         dx_VHF = 2.d0*delta*D_eff*((DEXP(-delta*y(1)))
c      &                -(DEXP(-2.d0*delta*y(1))))

c         ddx_VHF = -2.d0*(delta**2)*D_eff*((DEXP(-delta*y(1)))
c      &           -2.d0*(DEXP(-2.d0*delta*y(1))))


        yp(1) = y(2)

        yp(2) = -kappa*y(1)-kappa*(y(1)**3)*(1+epsiln*DCOS(t))
